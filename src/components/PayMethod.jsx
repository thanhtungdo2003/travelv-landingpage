...
import { createOrder } from '../services/AccountService';
  ...
  return (
    <StyledWrapper>
      <div className="paymethod-container">
        <form className="paymethod-form">
          <div className="separator">
            <hr className="line" />
            <p>Choose your pay method</p>
            <hr className="line" />
          </div>
          <div className="payment--options">
            <button className={method != 'paypal' || 'method-checked'} name="paypal" type="button"
            ...
            <button className={method != 'apple-pay' || 'method-checked'} name="apple-pay" type="button"
            ...
            <button className={method != 'google-pay' || 'method-checked'} name="google-pay" type="button"
            ...
            <div className={`paymethod-button ${method != 'momo' || 'method-checked'}`}
              onClick={() => setMethod('momo')}
            >
              <div className='paymethod-icon'>
                <img src='./momo.png' />
              </div>
              <div className='title'>EWallet MoMo</div>
              <div>
                <input type='radio' />
              </div>
            </div>
            <div className={`paymethod-button ${method != 'vnpay' || 'method-checked'}`}
              onClick={() => setMethod('vnpay')}
            >
              <div className='paymethod-icon'>
                <img src='./vnpay.jpg' />
              </div>
              <div className='title'>EWallet VNPay</div>
              <div>
                <input type='radio' />
              </div>
            </div>
          ...
          <div style={{
            ...
            <div className="credit-card-info--form" style={{
              display: method == "default" ? 'none' : 'flex'
            }}>
          ...
          <div className="purchase--btn" onClick={() => {
            const fetch = async () => {
              if (method == "default") {
                return toast.info('Choose your pay method')
              }
              const res = await createOrder(method, pack);
              if (res.status && res.status == 401){
                toast.info("You need to login");
                nav('/signin')
              }
              if (res.payUrl) {
                window.open(res.payUrl, '_current')
              }
              if (res.approve_url){
                window.open(res.approve_url, '_current')
              }
            }
            fetch();
          }}>Checkout</div>
